...
class TCPServer:
    ...
    def _hs_worker(self, cid, client_temp_pub):
        try:
            ...
            def send_enc(data):
                nonlocal sc; ctr = sc.to_bytes(8, "big"); sc += 1
            ...
            def recv_enc():
                nonlocal rc; payload = self._handshakes[cid].get(timeout=5)
                ctr = payload[:8]; rc += 1
            ...
            try:
                if poller.poll(100):
                    ...
                    if cid not in self._keys:
                        ...
                        continue
                    with self._lock:
                        ekey, sc, rc = self._keys[cid], self._counters[cid][0], self._counters[cid][1]
    ...
    def send(self, payload, eid=None, client_id=None):
        ...
        with self._send_lock:
            self.sock.send_multipart([cid, eid, ctr + Encryption.encryptGCM(payload, ekey, aad=eid + ctr + b"1")])
    def close(self): self._running = False; self.sock.close()
class TCPClient:
    def __init__(self, host, port, auth_key="./auth_key"):
        ...
        key = open(auth_key, "rb").read()
        with self._send_lock: self.sock.send_multipart([HANDSHAKE_EID, Encryption.encryptGCM(Encryption.basic_kdf(key,b'',6), tss)])
        ...
        self.ekey = Encryption.kdf_fast(Encryption.shared_secret(keys[0], key), tss)
        tp = Encryption.gen_ed25519(True)
    ...
    def _send_enc(self, eid, payload):
        ctr = self.sc.to_bytes(8, "big"); self.sc += 1
