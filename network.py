...
class TCPServer:
    def __init__(self, host, port, auth_key_dir="./keys", keys=64, on_exchange=None):
        ...
        self._io_thread = threading.Thread(target=self._run_event_loop, daemon=True)
        ...
        try:
            while self._running:
                while not self._send_q.empty():
                    ...
                    cid, eid, frames = item
                    await self.sock.send_multipart([cid, eid, frames])
                    ...
                    if cid not in self._keys:
                        with self._lock:
                            if cid not in self._handshakes:
                                ...
                                self._handshakes[cid].put(payload)
                        continue
        ...
        try:
            while self._running:
                ...
                if self.sock in events and events[self.sock] == zmq.POLLIN:
                    ...
                    if self.ekey is None:
                        self._hs_q.put(payload)
                    else:
                        with self._lock:
                            q = self._pending.get(eid) if eid != HANDSHAKE_EID else self._hs_q
                        if q:
                            q.put(payload)
                        else:
